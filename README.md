...
```
 ...
 ГОЛОВА 2 (сорняки):
   ...
   - Сохраняет веса только от клиента 2
    ...
    Как удалось выяснить, backbone является общим слоем (одной из частей глобальной модели) с общим признаками на исходной картинке из датасета, от которых потом зависят весы локальных клиентов в головах. Поскольку общие признаки одни, нам не важно что по факту клиенты в разных головах отличаются друг от друга (классификация на разное число классов и т. д.) 
```
