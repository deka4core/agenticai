...
-  **DenseNet121** - общий backbone для извлечения признаков
...
class GlobalModel(nn.Module):
    def __init__(self):
        # Общий backbone - извлекает признаки из изображений
...
def fed_avg(w1, w2):
    ...
    for key in w1.keys():
        if 'backbone' in key:
            #  УСРЕДНЯЕМ только backbone
            # backbone учится от ОБОИХ задач
...
### Почему усредняем только backbone?
...
```
 ...
 ГОЛОВА 2 (сорняки):
   ...
   - Сохраняет веса только от клиента 2
    ...
    Как удалось выяснить, backbone является общим слоем (одной из частей глобальной модели)
    с общим признаками на исходной картинке из датасета, от которых потом зависят весы локальных клиентов в головах.
    Поскольку общие признаки одни, нам не важно, что по факту клиенты в разных головах отличаются друг от друга
    (классификация на разное число классов и т. д.).
```
...
На текущий момент в проекте реализованы одна независимая модель для классификации болезней томатов DenseNet121 и одна модель для обнаружения сорняков (EfficientDet-D0), 
а также глобальная мультизадачная модель на основе DenseNet121 с двумя специализированными головами, обученная с использованием федеративного обучения (20 раундов, FedAvg с усреднением только весов backboneа). 
В ближайших планах — расширение архитектуры до полноценной системы с четырьмя агентами (DenseNet121, MobileNetV2, YOLOv8, EfficientDet-D0), 
каждый из которых будет иметь общий backbone и две головы, что полностью соответствует архитектуре, описанной в статье; 
интеграция EfficientDet для детекции объектов с возвратом bounding boxes и вычислением метрик mAP@0.5 и mAP@0.95; 
проведение сравнительного анализа всех локальных моделей и глобальной модели с визуализацией графиков обучения; 
а также создание демонстрационного веб-приложения для загрузки изображений и получения предсказаний в реальном времени с визуализацией bounding boxes для обнаруженных сорняков.
